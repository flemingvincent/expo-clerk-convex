	...
	const panResponder = React.useRef(
		PanResponder.create({
			...
			onMoveShouldSetPanResponder: (evt, gestureState) => {
				// Only activate swipe if conditions are met
				if (!editable || !isInPlan || !onRemove) return false;
			...
			onPanResponderGrant: () => {
				setIsSwiping(true);
				// Add haptic feedback on swipe start
				Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
			},
            onPanResponderMove: (event, gestureState) => {
                // Clamp the swipe to only go left
			...
			onPanResponderRelease: (evt, gestureState) => {
				...
				if (gestureState.dx < SWIPE_THRESHOLD && gestureState.vx < -0.3) {
					// Consider velocity as well for smoother feel
					Animated.timing(swipeAnim, {
						toValue: -SCREEN_WIDTH,
						duration: 200,
						useNativeDriver: false, // Can't use native driver for unmounting animation
					}).start(() => {
						handleRemove();
						swipeAnim.setValue(0);
					});
